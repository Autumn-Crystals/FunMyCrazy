...
const FACTS = [
  { icon: CheckCircle, label: "ROUND 1 — FREE ENTRY", accent: true },
  { icon: Calendar, label: "DEADLINE: 30 AUG 2026" },
  { icon: Video, label: "ROUND 2 — ONLINE MEETING" },
  { icon: Mic, label: "ROUND 2 — 2 SEP 2026" },
];
    ...
    >
      ...
      <div className="relative mx-auto grid max-w-7xl items-center gap-12 px-4 sm:px-6 lg:grid-cols-[1.05fr_1fr] lg:gap-10 lg:px-8">
        <div className="animate-rise-in text-center lg:text-left">
          ...
          <div className="mx-auto mt-5 max-w-lg space-y-1.5 text-sm font-bold uppercase tracking-wider text-primary-foreground/80 lg:mx-0">
            {["PICK YOUR IDEA.", "BUILD IT WITH GEMINI.", "SUBMIT IT.", "GET SHORTLISTED.", "PITCH IT IN 60 SECONDS."].map((step) => (
              <p key={step} className="flex items-center gap-2 lg:justify-start justify-center">
          ...
          <ul className="mt-9 grid grid-cols-2 gap-2.5 sm:gap-3 lg:max-w-xl">
            {FACTS.map(({ icon: Icon, label, accent }) => (
              <li
                key={label}
                className={`flex items-center gap-2.5 rounded-2xl px-3.5 py-3 text-left text-xs font-bold uppercase tracking-wide ring-1 sm:text-sm ${
                  accent
                    ? "bg-accent text-accent-foreground ring-accent/50"
                    : "bg-primary-foreground/10 ring-primary-foreground/15"
                }`}
              >
                <Icon className={`size-4 shrink-0 ${accent ? "text-accent-foreground" : "text-accent"}`} />
